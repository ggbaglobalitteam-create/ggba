...
import { getPostPaymentRoute, resolveFlow } from '@/lib/applicationFlow';
...
export default function PaymentPage() {
    ...
    useEffect(() => {
        async function init() {
            ...
            if (!flow.requiresPayment) {
                router.push(getPostPaymentRoute(config));
                return;
            }
            setIsLoading(false);
        }
    ...
    const selectedItems = useMemo(() => {
        return SERVICE_OPTIONS.filter((service) => selectedServiceIds.includes(service.id));
    }, [selectedServiceIds]);
    ...
    const displaySelectedItems = useMemo(() => {
        return selectedItems.map((item) => {
            ...
            const label = otherServiceLabel.trim() || 'Other service';
            return {
                id: item.id,
                label,
                amount: normalizedOtherAmount,
            };
        });
    }, [selectedItems, otherServiceLabel, normalizedOtherAmount]);

    const handlePayNow = async () => {
        if (!method || !canSubmitPayment) return;
        ...
        try {
            ...
            let appId = applicationService.getCurrentApplicationId();
            if (!appId) {
                const list = await applicationService.listApplications();
                appId = list[0]?.id;
                if (appId) applicationService.setCurrentApplicationId(appId);
            }

            if (appId && totalAmount > 0) {
    ...
    return (
        <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="mb-8">
                <h1 className="text-3xl font-bold text-gray-900 mb-2 tracking-tight">Payment Details</h1>
                <p className="text-gray-500">Select one or more services and complete your payment securely.</p>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12">
                        ...
                        <div className="p-6 space-y-3">
                                ...
                                return (
                                    <label
                                        ...
                                        className={`flex items-start justify-between gap-4 p-4 rounded-xl border transition-all cursor-pointer ${
                                            checked ? 'border-[#C6A96A] bg-[#FDF9EF]' : 'border-gray-200 hover:border-gray-300 bg-white'
                                        }`}
                                    >
                                        <div className="flex items-start gap-3">
                                            ...
                                            <div>
                                                ...
                                                {service.disclaimer && (
                                                    <p className="text-xs text-gray-500 mt-0.5">{service.disclaimer}</p>
                                                )}
                                            </div>
                                        </div>
                            ...
                            {isOtherSelected && (
                                <div className="rounded-xl border border-[#C6A96A]/40 bg-[#FDF9EF] p-4 grid grid-cols-1 md:grid-cols-2 gap-3">
                                    <div>
                                        <label htmlFor="otherServiceLabel" className="text-xs uppercase tracking-wide font-semibold text-gray-500">
                                            Service Name
                                        </label>
                                        <input
                                            id="otherServiceLabel"
                                            type="text"
                                    ...
                                    <div>
                                        <label htmlFor="otherServiceAmount" className="text-xs uppercase tracking-wide font-semibold text-gray-500">
                                            Amount (EUR)
                                        </label>
                                        <input
                                            id="otherServiceAmount"
                                            type="number"
                                            min="0"
                                            step="0.01"
                                            placeholder="0.00"
                            ...
                            <div className="text-xs text-gray-500 pt-1">
                                <p>* Non-refundable.</p>
                                <p>** Please refer to the terms and conditions of the package.</p>
                            </div>
                        </div>
                    </Card>
                </div>
                ...
                <div className="space-y-6 order-2 lg:order-2">
                    <Card className="p-0 overflow-hidden border-0 shadow-[0_8px_30px_rgb(0,0,0,0.04)]">
                        <div className="bg-gradient-to-br from-blue-50 to-indigo-50/50 p-6 border-b border-blue-100/50 relative overflow-hidden">
                            <h2 className="text-lg font-semibold text-gray-900 mb-1 relative z-10">Order Summary</h2>
                            <p className="text-sm text-gray-500 relative z-10">Based on selected services</p>
                        </div>

                        <div className="p-6 space-y-4">
                            {displaySelectedItems.length === 0 && (
                                <p className="text-sm text-gray-500">No service selected yet.</p>
                            )}
              ...
              ${method === 'card'
                                ? 'border-[#C6A96A] bg-blue-50/10 shadow-[0_0_0_4px_rgba(24,144,255,0.05)] scale-[1.02]'
                                : 'border-gray-100 bg-white hover:border-gray-300 hover:shadow-sm'
                            }
            `}>
                            <input type="radio" name="payment" onChange={() => setMethod('card')} checked={method === 'card'} className="mt-2 mr-4 ml-1 w-4 h-4 border-gray-300 text-[#C6A96A] focus:ring-[#C6A96A] transition-all" />
            ...
            `}>
                            <input type="radio" name="payment" onChange={() => setMethod('bank')} checked={method === 'bank'} className="mt-2 mr-4 ml-1 w-4 h-4 border-gray-300 text-[#C6A96A] focus:ring-[#C6A96A] transition-all" />
                            <div className="flex items-center gap-4">
                    ...
                    <div className="pt-4">
                        ...
                        {!hasValidOther && (
                            <p className="text-xs text-rose-600 mt-3 font-medium">
                                Please provide service name and amount for the &quot;Others&quot; selection.
                            </p>
                        )}

                        <div className="flex items-center justify-center gap-2 mt-6 p-4 bg-emerald-50/50 rounded-xl border border-emerald-100/50">
                            <ShieldCheck className="w-5 h-5 text-emerald-500" />
                            <span className="text-xs font-medium text-emerald-700">Secure 256-bit SSL encrypted payment</span>
                        </div>
                    </div>
                </div>
            </div>
